...
class SelfPlayEngine:
    ...
    def _add_layer(self):
        ...
        self.beam_state.add_new_layer(next_layer)
        new_positions = self.position_queue.get_layer(next_layer, clone=True)
    ...
    def _handle_terminal_positions(self, terminal_mask: torch.Tensor, results: torch.Tensor):
        ...
        full_results = torch.zeros(
            terminal_mask.shape[0], dtype=results.dtype, device=self.device
        )
